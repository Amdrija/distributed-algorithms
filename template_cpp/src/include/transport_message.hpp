...
public:
    ...
    TransportMessage(Address address, const uint32_t id,
                     std::shared_ptr<char[]> payload, const uint64_t length,
                     bool is_ack = false)
        : id(id), payload(payload), address(address), length(length),
          is_ack(is_ack) {
    ...
    TransportMessage(Address address, const char *bytes, uint64_t length)
        ...
        std::memcpy(&this->is_ack, bytes + sizeof(this->id),
                    sizeof(this->is_ack));
        std::memcpy(payload.get(),
                    bytes + sizeof(this->id) + sizeof(this->is_ack),
                    length - sizeof(this->id) - sizeof(this->is_ack));
    }

    std::unique_ptr<char[]> serialize(uint64_t &serialized_length) {
        serialized_length =
            this->length + sizeof(this->id) + sizeof(this->is_ack);
