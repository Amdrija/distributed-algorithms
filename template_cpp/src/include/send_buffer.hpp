...
public:
    SendBuffer(HostLookup host_lookup, uint64_t initial_capacity) : capacity(initial_capacity), host_lookup(host_lookup)
    {
        for (auto host : host_lookup.get_hosts())
        {
            this->sizes.emplace(host, 0);
            this->buffers.emplace(host, std::unique_ptr<char[]>(new char[initial_capacity]));
            this->message_counts.emplace(host, 0);
        }
    }

    std::unique_ptr<char[]> add_message(TransportMessage message, uint64_t &payload_length)
    {
        ...
        {
            std::unique_ptr<char[]> buffer_payload(new char[this->sizes[host_id]]);
            std::memcpy(buffer_payload.get(), this->buffers[host_id].get(), this->sizes[host_id]);
            payload_length = this->sizes[host_id];

            std::memcpy(this->buffers[host_id].get(), &serialized_length, sizeof(serialized_length));
            std::memcpy(this->buffers[host_id].get() + sizeof(serialized_length), payload.get(), serialized_length);
        ...
        std::memcpy(this->buffers[host_id].get() + this->sizes[host_id], &serialized_length, sizeof(serialized_length));
        std::memcpy(this->buffers[host_id].get() + this->sizes[host_id] + sizeof(serialized_length), payload.get(), serialized_length);
        this->sizes[host_id] += sizeof(serialized_length) + serialized_length;
    ...
    std::vector<TransportMessage> deserialize(Address address, char *buffer, ssize_t received_length)
    {
        ...
        {
            std::memcpy(&message_length, buffer + start, sizeof(message_length));
            messages.push_back(TransportMessage(address, buffer + start + sizeof(message_length), message_length));
