...
public:
    ...
    void send(TransportMessage message) {
        ...
        sendto(socket_fd, payload.get(), serialized_length, 0,
    ...
    void start_receiving(std::function<void(TransportMessage)> handler) {
        std::cout << "Andrija3" << std::endl;
        handler(
            TransportMessage(this->address, 12, std::shared_ptr<char[]>(new char[0]), 0, false));
        std::cout << "Andrija4" << std::endl;
        char buffer[MAX_BUFFER_SIZE];
        sockaddr_in source;
        socklen_t source_length = sizeof(source);

        auto received_length = recvfrom(socket_fd, buffer, MAX_BUFFER_SIZE, 0,
                                        reinterpret_cast<sockaddr *>(&source), &source_length);
        while (received_length >= 0) {
            std::cout << "Andrija5" << std::endl;
            handler(TransportMessage(Address(source), buffer, received_length));

            received_length = recvfrom(socket_fd, buffer, MAX_BUFFER_SIZE, 0,
                                       reinterpret_cast<sockaddr *>(&source), &source_length);
        }
