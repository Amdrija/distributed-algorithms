...
public:
    ...
    FairLossLink(Address address, HostLookup host_lookup) : send_buffer(host_lookup, MAX_SEND_BUFFER_SIZE), address(address)
    {
    ...
    void start_receiving(std::function<void(TransportMessage)> handler)
    {
        ...
        auto received_length = recvfrom(this->socket_fd, buffer, MAX_RECEIVE_BUFFER_SIZE, 0,
                                        reinterpret_cast<sockaddr *>(&source), &source_length);
        while (received_length >= 0)
        {
            std::vector<TransportMessage> messages = this->send_buffer.deserialize(Address(source), buffer, received_length);
            for (auto m : messages)
            {
                handler(m);
            }
            ...
            received_length = recvfrom(this->socket_fd, buffer, MAX_RECEIVE_BUFFER_SIZE, 0,
                                       reinterpret_cast<sockaddr *>(&source), &source_length);
        }
